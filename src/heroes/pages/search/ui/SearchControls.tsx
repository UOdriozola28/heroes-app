...
export const SearchControls = () => {
  ...
  const handleSetActiveAdvancedFilter = () => {
    setSearchParams(prev => {
      const activeOrNot = activeAccordion === 'advance-filters'
      if (activeOrNot) prev.delete('active-accordion'); else prev.set('active-accordion', 'advance-filters')
      return prev
    })
  }

  const handleChangeValueSlider = (e: number[]) => {
    setSearchParams(prev => {
    ...
    <>
      <div className="flex flex-col lg:flex-row gap-4 mb-8">
        ...
        <div className="flex gap-2">
          ...
          {/* show click */}
          <Button variant={activeAccordion === 'advance-filters' ? 'default' : 'outline'} className="h-12" onClick={handleSetActiveAdvancedFilter}>
              ...
              <div className="mt-4">
                ...
                <div className="relative flex w-full touch-none select-none items-center mt-2">
                  <Slider defaultValue={[selectedStrength]} max={10} step={1}
                    onValueChange={handleChangeValueSlider}
                  />
                </div>
              </div>
            </div>
